...
export function CliAuthPage() {
  ...
  const authReturnPath = useMemo(() => `${basePath}${basePath.includes("?") ? "&" : "?"}authReturn=1`, [basePath]);
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <Button asChild className="mt-4">
            <Link to={`/auth?next=${encodeURIComponent(authReturnPath)}&mode=${encodeURIComponent(authMode)}`}>Sign in / Create account</Link>
          </Button>
        </div>
      </div>
    );
